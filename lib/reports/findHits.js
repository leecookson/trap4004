...
//    item.totalMightLost = formatRes(item.totalMightLost);
    _.each(item.hits, function (hit) {
      if (hit.boxContent) {
        ...
        hit.stats.maxLootSym = maxLootSymbol(hit.boxContent.loot) || '';
        console.log(hit.stats.maxLootSym);
      ...
      hit.s0defBoostSym = boostSymbol(hit.boxContent.s0defBoost, 'def');
      hit.s0atkBoostSym = boostSymbol(hit.boxContent.s0atkBoost, 'att');
      hit.s1defBoostSym = boostSymbol(hit.boxContent.s1defBoost, 'def');
      hit.s1atkBoostSym = boostSymbol(hit.boxContent.s1atkBoost, 'att');
      hit.s0hero = shortenHeroLvl(hit.boxContent.s0KCombatLv);
      hit.s1hero = shortenHeroLvl(hit.boxContent.s1KCombatLv);
    });
...
function addAllianceHit(allianceHit, hit, users, alliances, incoming) {
  ...
  if (report) {
    report.by = user1.n;
    report.on = user0.n;
    if (!allianceHit.reports) {
      allianceHit.reports = [];
    }
    allianceHit.reports.push(report);
  }
